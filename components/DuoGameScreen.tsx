...
import { useDuoGameLogic } from '../hooks/useDuoGameLogic'; // Nya hooken
...
export default function DuoGameScreen({
  ...
  const {
    ...
    saveAndEndTurn,
    resetTurnState,
    switchPlayerTurn, // Importera den nya funktionen
  } = useDuoGameLogic({
    player1Name: player1,
    player2Name: player2,
    onNewCardNeeded: () => {
  ...
  useEffect(() => {
    ...
    // Städa upp timern om komponenten avmonteras eller state ändras
    return () => {
      ...
      <Box borderWidth={1} borderRadius="$lg" p="$3" my="$2" w="$full" bg={isCurrentPlayer ? "$primary100" : "$backgroundLight100"} borderColor={isCurrentPlayer ? "$primary300" : "$borderLight300"} sx={{ _dark: { bg: isCurrentPlayer ? "$primary900" : "$backgroundDark800", borderColor: isCurrentPlayer ? "$primary700" : "$borderDark700" } }}>
        ...
        <HStack flexWrap="wrap" mt="$2">
          {yearsToDisplay.map((year, idx) => {
            // FIX: isPrelim-variabeln är tillbaka
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {showBack && card && (
        <VStack space="md" alignItems="center" w="$full">
          ...
          {wasCorrect ? (
            ...
            // NYTT: Visar "Klar"-knapp vid fel svar
            <VStack alignItems="center" w="$full" mt="$2" space="sm">
      ...
      <TouchableOpacity onPress={onBackToMenu} style={{ marginTop: 16 }}><Text underline color="$textLight500" sx={{_dark: {color: "$textDark500"}}}>Avsluta till meny</Text></TouchableOpacity>
    </ScrollView>
  );
}
